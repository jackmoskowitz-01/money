...
const MeetingsOverview = () => {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Meeting Pipeline */}
      <Card className="border-border bg-card p-4">
        <h3 className="font-display text-sm font-bold mb-1">Meetings Pipeline</h3>
        <p className="text-[10px] text-muted-foreground mb-4">Prospects with meetings set or held across the team</p>

        {meetingProspects.length === 0 ? (
          <p className="text-xs text-muted-foreground text-center py-4">No active meetings in pipeline</p>
        ) : (
          <div className="space-y-2">
            {meetingProspects.map((prospect, i) => (
              <motion.div
                key={`${prospect.tenantId}-${prospect.buildingId}`}
                initial={{ opacity: 0, y: 5 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: i * 0.04 }}
              >
                <Link
                  to={prospect.tenant ? `/building/${prospect.buildingId}/tenant/${prospect.tenantId}` : '#'}
                  className="flex items-center gap-3 rounded-md border border-border bg-secondary/20 p-3 hover:border-primary/30 hover:bg-secondary/40 transition-all group"
                >
                  {/* Broker */}
                  {prospect.broker ? (
                    <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[10px] font-bold ${prospect.broker.color}`}>
                      {prospect.broker.initials}
                    </div>
                  ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-xs font-semibold text-foreground truncate">{prospect.displayName}</p>
                      <Badge
                        variant="outline"
                        className={`text-[9px] px-1.5 py-0 shrink-0 ${
                          prospect.stage === 'meeting_set'
                    ...
                    <p className="text-[10px] text-muted-foreground truncate">
                      {prospect.broker?.name && `${prospect.broker.name} · `}
                      {prospect.building?.name || ''}
                      {prospect.tenant ? ` · ${prospect.tenant.sqft.toLocaleString()} SF` : ''}
                    </p>
                    <p className="text-[10px] text-muted-foreground/50 mt-0.5">
                      <Clock className="inline h-2.5 w-2.5 mr-0.5" />
                      Last activity: {formatDate(prospect.lastActivity)}
                    </p>
                  </div>
